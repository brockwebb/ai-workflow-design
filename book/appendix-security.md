...
The boundary is not capability: it is autonomy over external state. NIST's Center for AI Standards and Innovation describes an AI agent system as a generative AI model combined with software scaffolding that enables planning and actions affecting real-world systems or environments {cite:p}`nist_caisi_rfi_2026`. In practice, this means file writes, API calls, database mutations, and network actions. Standalone chatbots and retrieval-augmented generation systems that do not take autonomous actions fall outside this boundary.
...
**Agent goal hijack (ASI01).** An agent processing external content (a document, a web page, a dataset) encounters adversarial instructions embedded in that content and deviates from its assigned task. This is indirect prompt injection applied to an agent with tool access. The risk is not that the agent produces wrong text; it is that the agent takes wrong actions: writing files, calling APIs, or querying databases in ways the operator never intended. Research pipelines that ingest documents from external sources and then act on them are the target environment for this attack class. For pipelines using the Model Context Protocol or similar tool-calling frameworks, the tool outputs themselves become the injection surface: any tool that returns external content can carry adversarial instructions back to the agent.

**Tool Misuse and Exploitation (ASI02).** An agent is tricked into using its granted tools in unintended ways. The mitigation is straightforward: the fewer tools an agent has access to and the more narrowly scoped each tool is, the smaller the misuse surface. This is a direct argument for designing tool APIs with minimal capability footprint (not for performance, but because tool scope is the primary governance control available to practitioners without platform-level security infrastructure).

**Cascading Failures (ASI08).** In multi-agent systems or pipelines where one agent's output feeds another's input, errors and compromises propagate. Each downstream agent treats the upstream output as ground truth. This is the multi-agent version of garbage-in, garbage-out, but across trust boundaries where each agent may amplify errors through its own reasoning. The governance implication: inter-agent outputs need the same validation discipline as external data inputs.

Two additional threats are worth understanding even if they are less directly within a practitioner's control. Identity and Privilege Abuse (ASI03; agents impersonating other agents or escalating access through identity weaknesses) motivates the identity infrastructure discussion below. Memory and Context Poisoning (ASI06; corruption of an agent's persistent state to distort reasoning) is relevant to any system using vector databases, conversation histories, or structured memory stores; poisoned context produces systematically wrong outputs that may not be detectable without explicit provenance tracking.
...
The NIST Cyber AI Profile makes the recommendation explicit: give AI systems unique and traceable identities and credentials to track their activity {cite:p}`nist_cyber_ai_profile_2025`. The profile highlights this under its Secure and Thwart focus areas as a high-priority recommendation. For practitioners, this means each pipeline component that acts autonomously should have its own identity, not a shared service account.

The practical gap is that mature, standardized identity infrastructure for agents does not yet exist. The NIST AI Agent Standards Initiative, launched in February 2026, is investing in research on agent authentication and identity infrastructure {cite:p}`nist_aasi_2026`. The NCCoE concept paper identifies OAuth 2.0/2.1, OpenID Connect, and the Model Context Protocol as candidate standards under evaluation. As of early 2026, the comment periods for both the NCCoE paper and the CAISI RFI had closed. Implementation guidance was estimated at 12 to 24 months from that baseline.
...
**Map to what they know.** The NIST Cybersecurity Framework (GOVERN, IDENTIFY, PROTECT, DETECT, RESPOND, RECOVER) is the framework your CISO already uses {cite:p}`nist_csf_2024`. The Cyber AI Profile maps AI-specific considerations to those functions {cite:p}`nist_cyber_ai_profile_2025`. You do not need to master the mapping. You need to show that your controls (logging, least privilege, human checkpoints, supply chain awareness) map to their categories.
...
As of early 2026, each of the five sources reviewed for this appendix was at a distinct stage in the standards pipeline. The CAISI RFI had closed its comment period and was superseded by the broader NIST AI Agent Standards Initiative. The NCCoE concept paper on agent identity had been published as a concept paper and was progressing beyond initial draft status. The OWASP Agentic Top 10 had completed its first public release. The Cyber AI Profile was an initial public review draft (IPRD). The NIST AI Agent Standards Initiative, launched February 2026, was actively convening stakeholders and had not yet produced implementation guidance.
