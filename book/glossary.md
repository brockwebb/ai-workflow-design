...
: The process by which an extraction or knowledge graph pipeline produces authoritative-looking structured output without the verification infrastructure to back it up. Structural appearance (nodes, edges, typed relationships) creates confidence that the content is verified, when the content is actually the output of an unvalidated stochastic process. See Chapters 3 and 4.
